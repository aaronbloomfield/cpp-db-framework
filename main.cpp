...
int main(int argc, char** argv) {
	...
	hfiles.push_back(*it + ".h");
        ...
        for ( int i = 0; i < fields.size(); i++ )
            hfile << "  " << convertType(types[i]) << " " << fields[i] << ";\n";
        hfile << "  bool";
        for ( int i = 0; i < fields.size(); i++ )
            hfile << " _" << fields[i] << "_is_null" << ((i!=fields.size()-1)?",":";\n");
        hfile << "\npublic:\n  " << *it << "();\n";
        ...
        // getters and setters
        for ( int i = 0; i < fields.size(); i++ ) {
            hfile << "  " << convertType(types[i]) << "* get" << tocapcase(fields[i]) << "();\n" <<
                    "  void set" << tocapcase(fields[i]) << " (" << convertType(types[i]) << " x);\n";
            if ( convertType(types[i]) != "char*" )
                hfile << "  void set" << tocapcase(fields[i]) << " (char* x);\n";
            hfile << "  void " << "set" << tocapcase(fields[i]) << "ToNull();\n";
        }
        ...
        cfile << ") {\n";
        for ( int i = 0; i < fields.size(); i++ )
            cfile << "  set" << tocapcase(fields[i]) << " (_" << fields[i] << ");\n";
        cfile << "}\n\n";
        // setAll() with strings
        cfile << "void " << *it << "::setAll (";
        for ( int i = 0; i < fields.size(); i++ )
        ...
        for ( int i = 0; i < fields.size(); i++ )
            cfile << "  if ( _" << fields[i] << " == \"NULL\" )\n    set" 
                    << tocapcase(fields[i]) << "ToNull();\n  else\n    set"
                    << tocapcase(fields[i]) << " ((char*)_" << fields[i] << ".c_str());\n";
        cfile << "}\n\n";
        // enter() with strings
        cfile << "void " << *it << "::enter(";
        for ( int i = 0; i < fields.size(); i++ )
        ...
        for ( int i = 0; i < fields.size(); i++ ) {
            ...
            // getter
            cfile << convertType(types[i]) << "* " << *it << "::get" << tocapcase(fields[i])
                    << "() {\n  if (_" << fields[i] 
                    << "_is_null )\n    return NULL;\n  else\n    return &" 
                    << fields[i] << ";\n}\n\n";
            cfile << "void " << *it << "::set" << tocapcase(fields[i]) << " (" 
                    << convertType(types[i]) << " x) {\n  " << extraCheck << fields[i] 
                    << " = x;\n  _" << fields[i] << "_is_null = false;\n}\n" << endl;
            // set to NULL
            cfile << "void " << *it << "::set" << tocapcase(fields[i]) << "ToNull() {\n  _"
                    << fields[i] << "_is_null = true;\n}\n\n";
            // setting from a char* when the field type is not char*
            if ( convertType(types[i]) != "char*" )
                cfile << "void " << *it << "::set" << tocapcase(fields[i]) << " (char* x) {\n"
                        << "  if ( x == NULL )\n    _" << fields[i] 
                        << "_is_null = true;\n  else {\n    "
        ...
        cfile << "dbobject* " << *it << "::readInFullRow(MYSQL_ROW row) {\n"
                << "  " << *it << " *ret = new " << *it << "();\n";
        for ( int i = 0; i < fields.size(); i++ )
            cfile << "  ret->set" << tocapcase(fields[i]) << "(row[" << i << "]);\n";
        cfile << "  return ret;\n}\n\n";
        // put()
